...
本项目旨在设计并实现一个用于CIFAR-10图像分类的全连接神经网络。我们系统性地探索了不同网络架构、损失函数、正则化方法以及优化算法对模型性能的影响。通过使用 Weights & Biases 工具进行高效的贝叶斯超参数搜索与Hyperband早停，我们确定了一组最优配置。最终，我们的最佳模型在CIFAR-10测试集上达到了 **71.49%** 的准确率。实验结果表明，一个兼具宽度与深度的网络架构 (1024, 1024, 512) 是有效学习特征的关键；其次，Adam优化器与Cosine退火学习率调度器的组合在收敛速度和最终性能上表现最佳；最后，适度的Dropout (~0.3) 是平衡大型模型表达能力与泛化性能最重要的正则化手段。
...
2. AdamW在本次实验中效果不如Adam。查阅资料发现，Adam与AdamW的核心区别在于权重衰减的实现方式。在Adam中，权重衰减与学习率是耦合的，其有效强度会随训练动态变化；而在AdamW中，权重衰减是解耦且恒定的。我们的实验结果表明，不同优化器的最优超参数区间可能存在显著差异，在我们设定的超参数搜索空间内，Adam那种动态的、与学习率耦合的正则化方式，为模型找到了一个泛化性能更优的解。

**scheduler的对比：**
![[Pic/Pasted image 20251102115630.png]]
**结果分析**:
1. 在整个训练过程中，Cosine最为稳定、平滑，并且持续上升，最终取得了最高的验证集准确率。这完美地体现了其设计哲学：通过平滑且持续的衰减，保证模型在训练后期能够稳定地在最优解区域进行精细微调，最终实现最佳的收敛效果。
2. OneCycle策略在训练的前10k步表现出极大的不稳定性，准确率波动非常剧烈，甚至出现了大幅下跌。这对应于其学习率快速爬升至最高点的“热身”阶段，这种激进的探索虽然可能跨越局部最优点，但也带来了巨大的风险，导致大部分训练过程被早停。在度过这个动荡期后，它的性能迅速回升，但在训练后期与Cosine收敛精度一致。
3. Plateau (平台期衰减)展现了不错的收敛速度。然而，它的上限也明显低于Cosine，这种被动式的调整策略在帮助模型冲击最高性能方面能力有限。
4. Cosine_wr（带重启的余弦退火）的起步速度明显慢于领先的Cosine和OneCycle，在整个训练过程中始终处于第二梯队。理论上，周期性的学习率重启旨在帮助模型跳出局部最优点，但在实践中，这种剧烈的扰动似乎完全破坏了平滑收敛的进程。每当模型可能要开始精细微调时，学习率的突然提升就将其“踢出”了那个区域。在相同的训练步数下，其效果不如传统的Cosine策略。

---

### **4. 最佳模型与性能评估**
通过对所有实验运行的分析，我从W&B中筛选出在验证集上准确率最高的模型,重新训练了 3000epochs。其超参数配置如下：
...
| `optimizer`     | `adam`                   |
| `scheduler`     | `cosine`                 |
| `learning_rate` | `0.0006093076917331049`  |
| `dropout`       | `0.31054967601076744`    |
| `weight_decay`  | `0.00043791002112156415` |
| **验证集最高准确率**    | **71.06%**               |
| **测试集准确率**      | **71.49%**               |
![[Pic/Pasted image 20251102134413.png]]

![[Pic/Pasted image 20251102134559.png]]

**结果分析**:
混淆矩阵直观地展示了模型在各个类别上的性能。从图中可以看出，模型在airplane(757/1000), automobile(819/1000), ship(803/1000) 和 horse(797/1000) 等轮廓鲜明、特征独特的类别上表现出色，正确率很高。
最主要的混淆发生在视觉特征相似的类别之间。例如，有194个cat被错误地识别为了dog，同时有159个dog被错认为cat。同样，大量的truck(105个)被错分为automobile。
这种混淆模式暴露了全连接网络的根本弱点：它将图像展平为一维向量，完全忽略了像素之间的空间结构信息。因此，它难以学习到纹理、形状等对区分猫狗至关重要的局部空间特征。

**定性结果：**
![[Pic/Pasted image 20251102134606.png]]
...
我的最佳模型最终在测试集上取得了 **71.49%** 的准确率，完成了本次作业的任务。
