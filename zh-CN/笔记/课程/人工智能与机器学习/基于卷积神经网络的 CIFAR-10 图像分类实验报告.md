...
首先，我们将一个基础CNN模型（例如，一个中等大小的残差网络，无高级增强）的性能与A1中的最佳FCN模型进行对比。


`[在此处插入CNN与FCN的训练/验证准确率对比曲线图]`

**结果分析**: 如图所示，即使是基础的CNN模型，在验证集上的准确率也轻松超过了A1中经过精细调优的FCN模型（例如，从71.49%提升至约85%）。这直观地证明了卷积操作在提取图像空间特征方面的压倒性优势。FCN的学习曲线在后期趋于平缓，表明其模型容量已达瓶颈；而CNN的学习曲线则展现出更高的上限和更强的学习潜力。

#### **3.2 模型架构探索**
**3.2.1 网络深度与残差结构的影响**

`[在此处插入不同深度/宽度或有/无残差结构模型的性能对比图]`
